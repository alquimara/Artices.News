...
type Secret=string| string[] | undefined|Buffer
  ...
  export default async (req:NextApiRequest, res:NextApiResponse)=>{
    ...
    if(req.method === 'POST'){
      ...
      const secret: Secret = req.headers['stripe-signature']
      let event: Stripe.Event;
      try {
        event = stripe.webhooks.constructEvent(buf, secret, process.env.STRYPE_WEBHOOKS_SECRET)  
      } catch (error) {
